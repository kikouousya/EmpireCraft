...
static bool RemoveAssetList(UVrmAssetListObject *&assetList) {
	...
	if (assetList->SkeletalMesh) {
		RemoveObject(assetList->SkeletalMesh->Skeleton);
		RemoveObject(assetList->SkeletalMesh->PhysicsAsset);
...
bool ULoaderBPFunctionLibrary::LoadVRMFileFromMemory(const UVrmAssetListObject *InVrmAsset, UVrmAssetListObject *&OutVrmAsset, const FString filepath, const uint8 *pData, size_t dataSize) {
...

	{
		...
		RenewPkgAndSaveObject(out->SkeletalMesh->Skeleton, b);
		RenewPkgAndSaveObject(out->SkeletalMesh->PhysicsAsset, b);
...
bool ULoaderBPFunctionLibrary::CopyPhysicsAsset(USkeletalMesh *dstMesh, const USkeletalMesh *srcMesh, bool bResetCollisionTransform){
	...
	auto  &srcPA = srcMesh->PhysicsAsset;
...
bool ULoaderBPFunctionLibrary::CreateTailBone(USkeletalMesh *skeletalMesh, const TArray<FString> &boneName) {
	...
	TArray<int32> tail;
	for (auto &b : boneName) {
		int first = skeletalMesh->RefSkeleton.FindBoneIndex(*b);
		if (first < 0) {
			continue;
		}
	...
	USkeleton *k_tmp = skeletalMesh->Skeleton;//NewObject<UVrmSkeleton>(GetTransientPackage(), NAME_None, EObjectFlags::RF_Public | RF_Transient);
	...
	*/
	...
	for (int i = 0; i < tail.Num(); ++i) {
		...
		GetDirectChildBonesLocal(sk_tmp->RefSkeleton, tail[i], children);
		if (children.Num()) {
			tail.RemoveAt(i);
	...
	FReferenceSkeletonModifier RefSkelModifier(sk_tmp->RefSkeleton, sk_tmp->Skeleton);
	for (int i = 0; i < tail.Num(); ++i) {
		FMeshBoneInfo b;
		FTransform t;

		b.ParentIndex = tail[i];
		b.Name = sk_tmp->RefSkeleton.GetBoneName(tail[i]);
		...
		t = sk_tmp->RefSkeleton.GetRefBonePose()[tail[i]];
	...
	if (bAdd) {
		sk_tmp->RefSkeleton.RebuildRefSkeleton(sk_tmp->Skeleton, true);

		skeletalMesh->Skeleton->ClearCacheData();

		skeletalMesh->Skeleton->MergeAllBonesToBoneTree(sk_tmp);
...

		{
			...
			{
				int boneNum = skeletalMesh->Skeleton->GetReferenceSkeleton().GetRawBoneNum();
