...
bool VRMConverter::ConvertModel(UVrmAssetListObject *vrmAssetList, const aiScene *mScenePtr) {
	...
	static int boneOffset = 0;
	{
		...
		//k->MergeAllBonesToBoneTree(src);
		{
			...
			VRMSkeleton::readVrmBone(const_cast<aiScene*>(mScenePtr), boneOffset, sk_tmp->RefSkeleton);
			...
			if (VRMConverter::Options::Get().IsPMXModel()) {
				...
				for (const auto& t : VRMUtil::table_ue4_pmx) {
					...
					for (int i = 0; i < 2; ++i) {
						...
						auto ind = sk_tmp->RefSkeleton.FindBoneIndex(*target);
						if (ind != INDEX_NONE) {
			...
			if (Options::Get().GetSkeleton()) {
				// update refpose
				FReferenceSkeletonModifier RefSkelModifier(sk->RefSkeleton, k);
				{
					FTransform t;
					t.SetIdentity();
					const auto &srcRef = sk_tmp->RefSkeleton;
					auto &dstRef = sk->RefSkeleton;
				...
				for (auto& w : meshWeight) {
					...
					if (f <= 254) {
						...
						if (f == 0) {
							...
							int dummy = 0;
							if (p) {
								dummy = sk->RefSkeleton.FindBoneIndex(UTF8_TO_TCHAR(p->mName.C_Str()));
								if (dummy == INDEX_NONE) {
									dummy = 0;
								}
							}
