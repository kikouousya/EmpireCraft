...
namespace {
	...
	void createSmallThumbnail(UVrmAssetListObject *vrmAssetList, const aiScene *mScenePtr) {
		...
		if (TextureID < 0 || TextureID >= (int)mScenePtr->mNumTextures) {
			return;
		}
		if (mScenePtr->mTextures[TextureID] == nullptr) {
			return;
		}
...

		{
			...
			auto *a = mScenePtr->mTextures[TextureID];
...
bool VRMConverter::ConvertTextureAndMaterial(UVrmAssetListObject *vrmAssetList, const aiScene *mScenePtr) {
	...
	MaskBoolTable.SetNum(mScenePtr->mNumTextures);
	...
	if (mScenePtr->HasTextures() && vrmAssetList->Textures.Num() != mScenePtr->mNumTextures) {
	...
	if (Options::Get().IsPMXModel()) {
		...
		int MatNum = mScenePtr->mNumMaterials;
	...
	if (mScenePtr->HasMaterials()) {
		...
		int MatNum = FMath::Max(1, (int)mScenePtr->mNumMaterials);

		// lastmaterial = gltf default material
		if (Options::Get().IsVRMModel()) {
			MatNum = FMath::Max(1, (int)mScenePtr->mNumMaterials - 1);
		}
			...
			//if (index >= 0) {
			{
				...
				if (dm) {
					...
					if (bMToon == false){
						...
						FString alpha = alphaMode.C_Str();
						if (alpha == TEXT("BLEND")) {
							if (Options::Get().IsNoTranslucent() == false) {
								dm->BasePropertyOverrides.bOverride_BlendMode = true;;
								dm->BasePropertyOverrides.BlendMode = EBlendMode::BLEND_Translucent;
							}
						}
