...
void VRMSkeleton::addIKBone(UVrmAssetListObject* vrmAssetList, USkeletalMesh *sk) {
	...
	// refskeleton
	auto& BonePose = sk->RefSkeleton.GetRefBonePose();
	FReferenceSkeletonModifier RefSkelModifier(sk->RefSkeleton, sk->Skeleton);
		...
		{
			{
				...
				int32 ind = 0;
				ind = sk->RefSkeleton.FindBoneIndex(*orgBone[i]);
				if (ind >= 0) {
					FTransform a;
					while (ind >= 0) {
						a *= BonePose[ind];
						ind = sk->RefSkeleton.GetParentIndex(ind);
					}
