...
#include "Character/CombatALSCharacter.h"
...
ACombatCharacter::ACombatCharacter(const FObjectInitializer& ObjectInitializer): Super(ObjectInitializer)
{
...
void ACombatCharacter::AddStartupGameplayAbilities()
{
...
void ACombatCharacter::RemoveStartupGameplayAbilities()
{
	check(AbilitySystemComponent);
...
void ACombatCharacter::OnItemSlotChanged(FRPGItemSlot ItemSlot, URPGItem* Item)
{
	RefreshSlottedGameplayAbilities();
}

void ACombatCharacter::RefreshSlottedGameplayAbilities()
{
...
void ACombatCharacter::FillSlottedAbilitySpecs(TMap<FRPGItemSlot, FGameplayAbilitySpec>& SlottedAbilitySpecs)
{
...
void ACombatCharacter::AddSlottedGameplayAbilities()
{
...
void ACombatCharacter::RemoveSlottedGameplayAbilities(bool bRemoveAll)
{
	TMap<FRPGItemSlot, FGameplayAbilitySpec> SlottedAbilitySpecs;
...
void ACombatCharacter::PossessedBy(AController* NewController)
{
	...
	if (InventorySource)
	{
		InventoryUpdateHandle = InventorySource->GetSlottedItemChangedDelegate().AddUObject(this, &ACombatCharacter::OnItemSlotChanged);
		InventoryLoadedHandle = InventorySource->GetInventoryLoadedDelegate().AddUObject(this, &ACombatCharacter::RefreshSlottedGameplayAbilities);
	}
...
void ACombatCharacter::UnPossessed()
{
...
void ACombatCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(ACombatCharacter, CharacterLevel);
}
...
float ACombatCharacter::GetMana() const
{
	return AttributeSet->GetMana();
}

float ACombatCharacter::GetMaxMana() const
{
	return AttributeSet->GetMaxMana();
}

float ACombatCharacter::GetMoveSpeed() const
{
	return AttributeSet->GetMoveSpeed();
}

int32 ACombatCharacter::GetCharacterLevel() const
{
	return CharacterLevel;
}

bool ACombatCharacter::SetCharacterLevel(int32 NewLevel)
{
...
bool ACombatCharacter::ActivateAbilitiesWithItemSlot(FRPGItemSlot ItemSlot, bool bAllowRemoteActivation)
{
...
void ACombatCharacter::GetActiveAbilitiesWithItemSlot(FRPGItemSlot ItemSlot, TArray<URPGGameplayAbility*>& ActiveAbilities)
{
	FGameplayAbilitySpecHandle* FoundHandle = SlottedAbilities.Find(ItemSlot);
...
bool ACombatCharacter::ActivateAbilitiesWithTags(FGameplayTagContainer AbilityTags, bool bAllowRemoteActivation)
{
	if (AbilitySystemComponent)
	{
...
void ACombatCharacter::GetActiveAbilitiesWithTags(FGameplayTagContainer AbilityTags, TArray<URPGGameplayAbility*>& ActiveAbilities)
{
	if (AbilitySystemComponent)
	{
...
bool ACombatCharacter::GetCooldownRemainingForTag(FGameplayTagContainer CooldownTags, float& TimeRemaining, float& CooldownDuration)
{
...
void ACombatCharacter::HandleDamage(float DamageAmount, const FHitResult& HitInfo, const struct FGameplayTagContainer& DamageTags, ACombatCharacter* InstigatorPawn, AActor* DamageCauser)
{
	OnDamaged(DamageAmount, HitInfo, DamageTags, InstigatorPawn, DamageCauser);
}

void ACombatCharacter::HandleHealthChanged(float DeltaValue, const struct FGameplayTagContainer& EventTags)
{
...
void ACombatCharacter::HandleManaChanged(float DeltaValue, const struct FGameplayTagContainer& EventTags)
{
	if (bAbilitiesInitialized)
	{
...
void ACombatCharacter::HandleMoveSpeedChanged(float DeltaValue, const struct FGameplayTagContainer& EventTags)
{
